...
ReviewBoard.prototype.uploadDiff = function(reviewRequestId, diff, callback) {
  ...
  var req = this._request(options, function(err, body) {
    if (err) return callback(err);
    return callback(null, body.diff);
  });
  var form = req.form();
  form.append('path', diff, {filename: 'patch.diff'});
};
...
ReviewBoard.prototype.createReviewRequestDraft = function(reviewRequestId, options, callback) {
  ...
  if (options.targetPeople) {
    form['target_people'] = options.targetPeople;
  }
  if (options.bugsClosed) {
    form['bugs_closed'] = options.bugsClosed;
  }
